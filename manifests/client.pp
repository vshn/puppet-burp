...
#
# [*server*]
#   Default: "backup.${facts['networking']['domain']}"
...
define burp::client (
  ...
  Stdlib::Fqdn              $server = "backup.${facts['networking']['domain']}",
  String                    $password = fqdn_rand_string(10),
  ...
  $_cron_hour = Array($cron_hour, true)
  $_cron_minute = Array($cron_minute, true)
